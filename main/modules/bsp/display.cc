...
bool display_init(bool default_dummy_draw)
{
    ...
    LvLock::registerCallbacks([](int timeout_ms) {
        ...
        auto& storage_service = NVSservice::requestInstance();
        storage_service.connectEventSignal([&](const NVSservice::Event& event) {
            if ((event.operation != NVSservice::Operation::UpdateNVS) || (event.key != NVSservice::SETTINGS_BRIGHTNESS))
            {
                return;
            }
            ...
            ESP_UTILS_CHECK_FALSE_EXIT(
                storage_service.getLocalParam(NVSservice::SETTINGS_BRIGHTNESS, value), "Get NVS brightness failed"
            );
        ...
        NVSservice::Value brightness = BRIGHTNESS_DEFAULT;
        if (!storage_service.getLocalParam(NVSservice::SETTINGS_BRIGHTNESS, brightness))
        {
            ESP_UTILS_LOGW("Brightness not found in NVS, set to default value(%d)", std::get<int>(brightness));
        }
        storage_service.setLocalParam(NVSservice::SETTINGS_BRIGHTNESS, brightness);
