...
#define EVENT_THREAD_NAME                   "nvs_service"
...
void NVSservice::Event::dump() const
{
    ESP_UTILS_LOGI(
        "{Event}:\n"
        "\t-Operation(%d)\n"
        ...
        _event_thread = boost::thread([this]() {
            ...
            while (true) {
                ...
                while (!_event_queue.empty()) {
                    ...
                    // processEvent最终会调用sentEvent，需要用到_event_mutex
                    lock.unlock();
...
bool NVSservice::sendEvent(const Event& event, EventFuture* future)
{
    ESP_UTILS_LOG_TRACE_GUARD_WITH_THIS();
...
bool NVSservice::setLocalParam(const Key& key, const Value& value, const void* sender, EventFuture* future)
{
    ESP_UTILS_LOG_TRACE_GUARD_WITH_THIS();
...
bool NVSservice::getLocalParam(const Key& key, Value& value)
{
    std::lock_guard<std::mutex> lock(_params_mutex);
...
bool NVSservice::eraseNVS(const void* sender, EventFuture* future)
{
    ESP_UTILS_LOG_TRACE_GUARD_WITH_THIS();
...
boost::signals2::connection NVSservice::connectEventSignal(EventSignal::slot_type slot)
{
    ESP_UTILS_LOG_TRACE_GUARD_WITH_THIS();
...
bool NVSservice::processEvent(const Event& event)
{
    ESP_UTILS_LOG_TRACE_GUARD_WITH_THIS();
...
bool NVSservice::doEventOperationUpdateNVS(const Key& key)
{
    ESP_UTILS_LOG_TRACE_GUARD_WITH_THIS();
...
bool NVSservice::doEventOperationUpdateParam()
{
    ESP_UTILS_LOG_TRACE_GUARD_WITH_THIS();
...
bool NVSservice::doEventOperationEraseNVS()
{
    ESP_UTILS_LOG_TRACE_GUARD_WITH_THIS();
